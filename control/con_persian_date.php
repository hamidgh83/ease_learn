...
class con_persian_date {
        ...
        function to_date($g_date, $input) {
                $g_date_org=$g_date;
                $g_date=date("Y-m-d h:i:A",strtotime($g_date));
                ...
                $sec=explode(' ', $g_date);
                $g_year = substr($sec[0], 0, 4);
                $g_month = substr($sec[0], 4, 2);
                $g_day = substr($sec[0], 6, 2);
                
                $g_hr=substr($sec[1], 0, 2);
                $g_min=substr($sec[1], 2, 2);
                $g_am_pm= substr($sec[1], 4, 2);
                $g_am_pm= $g_am_pm=='AM'?'ق.ظ':'ب.ظ';
	    ...
	    return $this->persian_day_names[date('w',strtotime($g_date_org))].' '.$persian_d.' '.$this->persian_month_names[$persian_date[1]].' '.$persian_date[0].' - ' . $g_hr.':'.$g_min.' '.$g_am_pm;
                }
                if ($input == 'just_date') {
	    $persian_d = $persian_date[2];
	    if ($persian_d{0} == '0')
	            $persian_d = substr($persian_d, 1);
	    return $this->persian_day_names[date('w',strtotime($g_date_org))].' '.$persian_d.' '.$this->persian_month_names[$persian_date[1]].' '.$persian_date[0];
                }
        }

        function date($input) {
                return $this->to_date(date('Y') .'-'. date('m') .'-'. date('d').' '.date('H') .':'. date('i') .':'. date('s'), $input);
        }
